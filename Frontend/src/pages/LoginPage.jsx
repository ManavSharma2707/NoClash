...
const InputField = ({ label, name, type = 'text', value, onChange, placeholder, required = true }) => (
    <div className="mb-4">
        ...
        <input
            className="w-full px-3 py-2 border rounded-md shadow-sm sm:text-sm text-gray-700 border-gray-300 focus:outline-none focus:ring-2 focus:ring-offset-1 focus:ring-indigo-500 focus:border-indigo-500 transition duration-150 ease-in-out"
            id={name}
            name={name}
            type={type}
...
function LoginPage() {
    ...
    return (
        <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-indigo-100 via-white to-purple-100 p-6">
            <div className="bg-white p-8 sm:p-10 rounded-xl shadow-lg w-full max-w-lg border border-gray-200">
                                {/* Brand Logo */}
                ...
                {error && <p className="bg-red-100 text-red-700 p-3 rounded mb-4 text-sm border border-red-300" role="alert" aria-live="polite">{error}</p>}
